...
When reviewing different platform we focused on free and open-source software (\tracknshrink{FOSS}) solutions, since this would allow to modify and extend functionality. Unfortunately, there is no platform that is \tracknshrink{FOSS} which provides also a substantial foundation for a liquid democracy
...
Although there are a few projects that implemented \tracknshrink{LD}---to various degrees---there is no project that offers access to its data. Additionally, we focus on free and open-source software (\tracknshrink{FOSS}) solutions, since this would allow to modify and extend functionality. Unfortunately, as of early 2019, there is no \tracknshrink{FOSS} that would off
