...
After \hyperref[ssec:Motivation]{motivating} the context of this research and \hyperref[ssec:Objective]{formulating the research question}, the \hyperref[sec:Theory]{next section} will give discuss more thoroughly the two major societal contexts this project is situated in, namely \hyperref[ssec:Theory_CS]{Citizen Science} and \hyperref[ssec:Theory_LD]{Liquid Democracy}. From the discussion of these theoretic aspects, we will derive a \hyperref[ssec:Criteria]{range of criteria} a platform for a Citizen Science-based Liquid Democracy game need to fulfill.

Subsequently, we will \hyperref[sec:RelatedWork]{explore existing approaches} for Liquid Democracy platforms (or voting platforms as is often the case), presenting the current state of the art. We will \hyperref[ssec:Evaluation]{evaluate} in how far these platforms conform to the \hyperref[ssec:Criteria]{criteria derived from the theory}. 

As will be shown, no existing approach is suited to fulfill the criteria. Thus, in \hyperref[sec:ProjectRequirements]{the following section} we will develop a platform based on the requirements sketched before. This will be done by \hyperref[ssec:SoftwareRequirements]{compiling product requirements} the platform needs to fulfill in order to meet the projects requirements. \hyperref[ssec:Implementation]{Subsequently}, we will describe the implementation of a prototype based on the software requirements set before.

Finally, we will \hyperref[sec:Conclusion]{conclude} by critically discussing the \hyperref[ssec:DiscussionImplementation]{prototype implementation} in the context of the project, with a particular focus on the \hyperref[ssec:Criteria]{criteria derived from the theory}, before we will conclude with a discussion of \hyperref[ssec:FutureWork]{future work}.