...
As will be shown, no existing approach is suited to fulfill the criteria. Thus, in \hyperref[ch:Our_Project]{the following section} we will develop a platform based on the requirements sketched before. This will be done by \hyperref[sec:SoftwareRequirements]{compiling product requirements} the platform needs to fulfill in order to meet the projects requirements. \hyperref[sec:Implementation]{Subsequently}, we will describe the implementation of a prototype based on the software requirements set before.
