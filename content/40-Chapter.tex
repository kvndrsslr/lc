...
It is clear that each natural person should not be able to obtain this privilege multiple times, which can be broken down to the requirements that (1) no natural person should be able to obtain more than one user account and (2) that no user account should be able to obtain the privilege of participation twice.
There is no easy way to solve (1) purely in the digital realm.
It should therefore be the responsibility of the platform operators to come up with a system which distributes unique user accounts to participants without the possibility of malicious persons to obtain more than one.
However, as has been stated in \autoref{sec:User_Intent_Emission}, a blind signature scheme should be utilized in order to preserve secrecy of user intents even against the central trusted authority which manages the privilege of participation.
