...
In this section we will discuss the theoretical background the project of this report is based on. As noted in \hyperref[ssec:Objective]{the introduction}, the project aims at developing a proof-of-concept implementation of a Citizen Science accessible Liquid Democracy game. As such, it needs to be grounded in a thorough understanding of both the concept of \hyperref[ssec:Theory_CS]{Citizen Science} and \hyperref[ssec:Theory_LD]{Liquid Democracy}. Additionally it needs to identify and address the problems and issues raised in relation to these concepts. 
...
Subsequently to a discussion on the background of the respective concept, we will \hyperref[ssec:Integration_CSLD]{discuss synergies and challenges} of integrating these perspectives. Based on this integrative perspective and the aspects discussed at the beginning of this section, we will \hyperref[ssec:Criteria]{derive a number of criteria} a Liquid Democracy game grounded in Citizen Science needs to exhibit, in order to \hyperref[sec:Approach]{develop} a theory-grounded approach to the project's objective.
% Clara
\section{CS}
\label{sec:Theory_CS}
...
\end{displayquote}
A delegation graph is a fundamental component of liquid democracy. subset  which is embedded within the concept of liquid democracy. \autoref{fig:Delegation-Casscade} visualizes a delegation graph/cascade. 

\begin{figure}[H]
\centering
\includegraphics{delegation-casscade.png}
	...
	\label{fig:Delegation-Casscade}
\end{figure}
Most Proxy voting mechanisms, however, 

is a delegation-only. By allowing a direct voting the field 

The limitation, however, of most proxy voting mechanisms is the lack direct voting possibilities. the an individual citizen may direct vote for a certain state of affairs (as seen in the delegation graph). This gap 
...
% TODO Noun-Project creditieren
\begin{figure}[H]
	\centering \begin{tikzpicture}
	\node[anchor=south west,inner sep=0] (image) at (0,0,0) {\includegraphics[width=15cm]{Liquid-Democracy-Delegated-Voting.pdf}};
	\begin{scope}[x={(image.south east)},y={(image.north west)}]
% 	% next four lines will help you to locate the point needed by forming a grid. comment these four lines in the final picture.↓
% 		\draw[help lines,xstep=.1,ystep=.1] (0,0) grid (1,1);
% 		\draw[help lines,xstep=.05,ystep=.05] (0,0) grid (1,1);
...
% 	% upto here↑
    ...
    \draw (0.045,1.03) node {\textsf{\textbf{Citizen}}};
    ...
    \draw (0.068,0.730) node {\textcolor{gray}{\(1\)}};
    \draw (0.068,0.571) node {\textcolor{gray}{\(1\)}};
    \draw (0.068,0.353) node {\textcolor{gray}{\(1\)}};
    \draw (0.068,0.193) node {\textcolor{gray}{\(1\)}};
    \draw (0.068,0.002) node {\textcolor{gray}{\(1\)}};
    
    \draw (0.280,0.618) node {\textcolor{gray}{\(3\)}};
    \draw (0.450,0.618) node {\textcolor{gray}{\(4\)}};
    \draw (0.450,0.232) node {\textcolor{gray}{\(3\)}};
