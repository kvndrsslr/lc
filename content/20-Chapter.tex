...
It is important to point out that the voting form \textit{proxy voting} shares some similarities of liquid democracy. There is, however, a fundamental difference: in \acrshort{LD} a proxy may further delegate his/her voices to another proxy, which is not intended for proxy voting. Thus, a proxy in the concept of liquid democracy can be---mathematically---described as a transitive (recursive) vertex within a directed graph. \citeauthor{Kahng2018} states:
...
\end{figure}
However, the option of direct voting is not intended in proxy voting and most delegation graphs. Therefore, to fulfill the initial definition of \acrshort{LD}, the delegation graph must be extended to permit direct voting. This enhancement allows to further classify the terms: proxy voting, delegation graph and liquid democracy in a true subset relationship: \textit{proxy voting \(\subsetneq\) delegation graph \(\subsetneq\) liquid democracy}. Therefore, the concept of liquid democracy is a superset of proxy voting and delegation graphs. 

\subsection{Liquid Democracy Mechanics}
\label{ssec:Liquid_Democracy_Mechanics}

In liquid democracy the directed delegation graph flows from a principal to a proxy. To visualize this process in a simplified manner we will discuss some key aspects of the vote flow. Hence, \autoref{fig:Liquid-Democracy-Delegated-Voting} illustrates the main steps of a voting process within a liquid democracy system. Initially, each citizen starts as a `primary principal' holding their own voice (i.e., a voting power of 1). From there, a citizen has three options: (1) to vote \textit{directly} for a given state of affairs, (2) to \textit{delegate} his/her voice to a proxy, or (3) to not participate at all. Option (1) entitles the citizen to directly influence with his/her opinion 
