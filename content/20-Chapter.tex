...
It is important to point out that the voting form \textit{proxy voting} shares some similarities of liquid democracy. There is, however, a fundamental difference: in \acrshort{LD} a proxy may further delegate his/her voices to another proxy, which is not intended for proxy voting.
...
\end{displayquote}
A delegation graph is a fundamental component of liquid democracy. \autoref{fig:Delegation-Cascade} visualizes such a delegation graph/cascade. 
...
\end{figure}
However, the option of direct voting is not intended in proxy voting and most delegation graphs. Thus, to fulfill the initial definition of \acrshort{LD}, the delegation graph must be extended to permit direct voting. This enhancement allows to further classify the terms: proxy voting, delegation graph and liquid democracy in a true subset relationship: \textit{proxy voting \(\subsetneq\) delegation graph \(\subsetneq\) liquid democracy}. Therefore, the concept of liquid democracy is a superset of proxy voting and delegation graphs. 

\subsection{Liquid Democracy Mechanics}
\label{ssec:Liquid_Democracy_Mechanics}

\autoref{fig:Liquid-Democracy-Delegated-Voting} illustrates a simplified voting process of a liquid democracy system. Initially, each citizen starts as a \textit{primary principal} holding their own voice (voting power of 1). In this position one has three options: (1) vote directly for a given state of affairs, (2) delegate your voice to a proxy, or (3) not participate at all. In case (1) we

Being a proxy you get defacto a principal in LD

Citizens may have the right to not vote at all 

Since we are in the domain of a directed graph, votes are flowing in one direction from a principal to a proxy. 

As in \autoref{fig:Delegation-Cascade} ‘voting power’ is
...
\end{figure}
More abstract from a principal to a proxy, where the principal can be a proxy, too.\footnote{Icons provided by thenounproject.com under Creative Commons \includegraphics[height=\fontcharht\font`\X]{cc-icon.pdf} license:\\
(1) \url{https://thenounproject.com/icon/1471169/}\\
(2) \url{https://thenounproject.com/icon/751412/}\\
(3) \url{https://thenounproject.com/icon/575955/}\\
(4) \url{https://thenounproject.com/icon/24402/}}. 
