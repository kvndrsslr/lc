...
As described in~\ref{sec:Model_Contexts}, subjects belong to different, sometimes multiple contexts. In order to allow for the versatility demanded there, but recognizing that a principle context (at some level within the taxonomy) is the focus of a proposition, we decide 

Propotatosite contextual the data through assigning a primary context and an arbitrary number of secondary contexts.iometans typically evolve ovto anner time which can be conceptualized as a succession of versioned snapshots.
...
However, the protection from structural analysis could be weakened, such that it would be theoretically possible for both actual citizen scientist \todo{not very security-by-designy which we mentioned}which we assume to not have evil intentions) and adversaries to deduce the political intentions of at least a few users.
