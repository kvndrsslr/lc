...
This section will describe a model of a Liquid Democracy grounded in Citizen Science. Based on the \hyperref[sec:Theory]{theoretic analysis} performed, we will sketch the relevant processes and entities within the platform. 

This includes the core processes, such as \hyperref[ssec:Model_VoteDelegation]{vote delegation} and \hyperref[ssec:Model_VoteFeedback]{vote feedback}, the \hyperref[ssec:Model_Propositions]{life cycle of propositions}, the \hyperref[ssec:UserRoles]{roles} users take on within the platform, the \hyperref[ssec:Model_Contexts]{geographical and political context} propositions take place in, the \hyperref[ssec:Model_Visualization]{visualization}, and, most importantly, \hyperref[ssec:Model_ResearchersAccess]{the accessibility of data for Citizen Scientists}. 
...
The petitioner is the role a user can take on when initiating the political process by starting a \hyperref[ssec:Model_Propositions]{proposition}. Since the process of propositions is already described in \ref{sec:Model_Propositions}, the following focuses on the role.
...
Users can also create alternative petitions to existing ones, thus becoming alternative petition authors (for the mechanics of alternative petitions see \ref{sec:Model_Propositions}). These petitions are assigned to the same petition group. For this petition the alternative petition author counts as \hyperref[sssec:Roles_Petitioner]{petition author}, and has the same rights as them for the same reasons. Since an alternative petition works basically the same way as the original petition, the differentiation between a petition author and an alternative petition author is merely semantical. 
...
The proposition modification requester is a \hyperref[sssec:Roles_DiscussionParticipant]{discussion participant} that raises a constructive petition text edit request. 
