...
Rather, we would like to involve the user to actively filter propositions based and therefore derive the fourth criteria for a \tracknshrink{LD} platform as \textbf{(\tracknshrink{LD}5)} active information filtering capabilities.
...
Finally, we derive the last criteria for a \tracknshrink{LD} platform from the discussion in \ref{ssec:Integration_AccessibilityAnonymity} as \textbf{(\tracknshrink{LD}7)} protection of privacy.
...
However, in order to follow the protection of privacy \textbf{(\tracknshrink{LD}7)}, users should be able to chose themselves which information they want to be displayed to the public.
...
Surely, one could imagine a system where the right to vote is granted based on consensus as proposed by the \cite{}\todo{@KD:insert citation}.
