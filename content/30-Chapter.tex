...
Based on our work in \ref{sec:Liquid_Democracy}, we derive the core functionalities of a \tracknshrink{LD} platform as \textbf{(\tracknshrink{LD}1)} collaborative editing of propositions, \textbf{(\tracknshrink{LD}2)}  secret and verifiable voting on propositions and \textbf{(\tracknshrink{LD}3)} vote delegation.
...
Apart from their actual information content, propositions possess additional metadata and dependency relationships to other information in the platform.
...
Since it is desirable that edits to propositions are funded on a discursive process involving multiple individuals, we argue that only the original creator of the proposition, i.e. its owner, should be able to directly edit it.
...
In order to meet the requirements of \textbf{(\tracknshrink{LD}2)} user intents should be anonymized to satisfy the secrecy property as well as public and authorized to satisfy the verifiability property.
