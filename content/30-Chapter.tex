...
Therefore, our contribution in this work is split in two parts: (1) a \tracknshrink{LD} platform design that can be universally used in multiple \tracknshrink{CS} projects and (2) a \tracknshrink{CS}-enabling interface which exposes data from the \tracknshrink{LD} platform in a way such that it will be easy to build specific \tracknshrink{CS} projects on top of it.

Based on our work in \ref{sec:Liquid_Democracy}, we derive the core functionalities of a \tracknshrink{LD} platform as \textbf{(\tracknshrink{LD}1)} collaborative editing of propositions, \textbf{(\tracknshrink{LD}2)} free, fair and secret voting on propositions and \textbf{(\tracknshrink{LD}3)} vote delegation.
...
In this chapter we will develop a conceptual overview of both a \tracknshrink{LD} platform and a \tracknshrink{CS}-enabling interface based on the criteria derived in \ref{sec:Criteria}.
To this end, we will start with the centerpiece of democratic discourse: propositions, their properties and lifecycle.
Then we describe the various roles users can take in the platform, before we describe voting and vote delegation mechanisms.
...
Apart from their actual information content, propositions possess a number of additional, implicit properties, their metadata.
In a digital realm this metadata is very important for the accessibility of the actual information content, for example through active filtering mechanisms as required by \textbf{(\tracknshrink{LD}4)}.
Amongst others, propositions have the following metadata:
\begin{itemize}
\item Creator
\item Time of creation
\item Political context(s)
\item Political administrative unit
\end{itemize}

Moreover, propositions typically evolve over time and therefore the snapshot of a proposition at a given time can be assigned an increasing version number.
...
Choosing a proper taxonomy to categorize propositions is crucial for a \tracknshrink{LD} project as it is a sort of foundation for the work to follow.
Certainly, clearly defined categories are important to make order of the ongoing topics and are essential to stay on top of things.
%In our cases however this taxonomy is essential as it is an important part of the voting process as well.
On the one hand the categories can help the user to find votes he is interested in, which partly satisfies criteria \textbf{(\tracknshrink{LD}4)}.
Even more important, with categories he can also determine for which issues he wants to vote himself and which topics he wants to pass on to a person with more expertise in this field.
Taking for example a vote about the deforestation of the south of Germany, it could fall into a category such as environment.
Imagining that the user himself is not that much of specialist when it comes to the environment and wants to delegate his votes for propositions on this subject to a relative who is.
...
The corpus of the open encyclopedia is often used to build automatic categorization approaches or to improve the performance of existing models.Seeing the pure size of the corpus and the fact that it changes constantly you don’t wonder why this is the case.
...
We note that in order to meet our third requirement, the platform should possess mechanisms for the collaborative editing of the taxonomy.
