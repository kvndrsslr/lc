\chapter{A Citizen Science Approach to Liquid Democracy}
% SK: habe mal das Processes in der Überschrift
\label{ch:Approach}

In the previous chapter we discussed the theoretical background for two core components for such an application: (1) a participatory system allowing users to either vote or to delegate their voices, which is scaffolding the other core component: (2) an interface allowing interested users to access the system’s data in order to perform analyses on various topics. In other words a user becomes a researcher. (1) is describing the concept of \textit{liquid democracy} and (2) the approach of \textit{citizen science}. It is important to grasp the idea for both components before merging them. Thus in \autoref{sec:Liquid_Democracy} we introduce the political pathway for liquid democracy and in \autoref{sec:Theory_CS} the theory behind \textit{citizen science}. At the end for each section we identify and address issues raised in relation to these concepts. 

This section will describe a model of a liquid democracy grounded in citizen science. Based on the theoretic analysis, we will sketch the relevant processes and entities within such a platform. This includes the core processes of liquid democracy, that are vote delegation and voting mechanism, the life cycle of propositions, user roles within the platform, the geographical and political context propositions take place in, the visualization, and, most importantly, the accessibility of data for citizen scientists. 
...
Overall, there are two requirements for \tracknshrink{CS}-based \tracknshrink{LD} system. (1)
The core component is a digital liquid democracy platform, supporting at least the two fundamental elements of \tracknshrink{LD}: direct and delegate voting. (2) Additionally, this platform requires an interface to access its data, at some extend, allowing citizen scientist to perform analyses. Apart from these two main requirements there are two more important aspects that a \tracknshrink{LD} platform should guaranty: (a) privacy protection (i.e., protection of the user’s personal identity) and (b) abuse protection (e.g., spam or identity fraud). We already discussed in \autoref{ssec:Integration_AccessibilityAnonymity} the significance of \nameref{ssec:Integration_AccessibilityAnonymity}, thus we want to emphasize the importance of abuse protection, briefly.
